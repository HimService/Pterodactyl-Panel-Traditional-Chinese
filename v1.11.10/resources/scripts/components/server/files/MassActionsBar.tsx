        ...
        <>
            <div css={tw`pointer-events-none fixed bottom-0 z-20 left-0 right-0 flex justify-center`}>
                ...
                >
                    <p className={'mb-2'}>
                        您確定要刪除&nbsp;
                        <span className={'font-semibold text-gray-50'}>{selectedFiles.length} 個檔案</span>嗎？這是一個永久性的操作，檔案將無法復原。
                    </p>
                    {selectedFiles.slice(0, 15).map((file) => (
                        <li key={file}>{file}</li>
                    ))}
                    {selectedFiles.length > 15 && <li>以及其他 {selectedFiles.length - 15} 個檔案</li>}
                </Dialog.Confirm>
                {showMove && (
                    <RenameFileModal
                        files={selectedFiles}
