        ...
        <div class="box box-success">
            ...
            <div class="box-body">
                <p class="text-muted small">
                    使用下面的按鈕產生自訂部署指令，該指令可用於透過單一指令在目標伺服器上設定 Wings。
                </p>
            </div>
            <div class="box-footer">
    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: '權杖已建立。',
                text: '<p>若要自動設定您的節點，請執行以下指令：<br /><small><pre>cd /etc/jexactyl && sudo wings configure --panel-url {{ config('app.url') }} --token ' + data.token + ' --node ' + data.node + '{{ config('app.debug') ? ' --allow-insecure' : '' }}</pre></small></p>',
                html: true
            })
        }).fail(function () {
            swal({
